...
If =:session= argument has a name, we will try to create a session which can match this name in =helm-q= if it can match via =heml-q=.
...
#+begin_src q :session host.dev :exports results
1+1
#+end_src
...
we will create a new local instance and rename its buffer to session name surrounding with =*=.
