...
in =instances-meta.json= are defined in variable =ob-q-remote-session-pattern=, which are =service,region,env= and
match in a strict format delimited with dot (.) as in :session,
for example session name should be =TAQ.HDB.prod.nam= if =service= is =TAQ.HDB=, =region= is =prod=,
and =env= is =nam=.
